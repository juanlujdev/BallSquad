...
export class StoreOrderComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.storeOrderService
      ...
      .subscribe({
        next: (res: HttpResponse<IStoreOrder[]>) => {
  ...
  trackId(index: number, item: IStoreOrder): number {
    return item.id!;
  }

  delete(storeOrder: IStoreOrder): void {
    const modalRef = this.modalService.open(StoreOrderDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.storeOrder = storeOrder;
  ...
  protected onSuccess(data: IStoreOrder[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
